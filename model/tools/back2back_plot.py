...
class bbplot(object):
    ...
    def __init__(self, conf, pdeep_model, plot_ion_types, tol = 20, tol_type = "ppm"):
        ...
        self.ion_color = {'b{}':'#1E1EE5','y{}':'#E51E1E','c{}':'darkgreen','z{}':'#9370db','b{}-ModLoss':'black','y{}-ModLoss':'black','b{}-H2O':'#1E1EE5','y{}-H2O':'#E51E1E','b{}-NH3':'#1E1EE5','y{}-NH3':'#E51E1E'}
        self.config = conf
    ...
    def batch_save(self, pf2, pep_list, save_dir):
        self.Readpf2(pf2)
        for pep in pep_list:
            fig, pcc = self.plot(*pep)
            plt.tight_layout()
            mng = plt.get_current_fig_manager()
            #mng.window.showMaximized()
            #mng.resize(*mng.window.maxsize())
            plt.savefig(os.path.join(save_dir, "%s-%s-%s-R=%.3f.png"%(pep[2], pep[0], pep[1], pcc)),format="png", dpi=220)
            plt.close()
    ...
    def pcc_batch(self, psmlist):
        ...
        pccs = []
        for psm in psmlist:
            fig, pcc = self.plot(peptide = psm[1], modinfo = '', spec = psm[0])
